package com.example.amukisense.ui.screens
...
import com.example.amukisense.ui.viewmodel.ComparativeGpsRecord
import com.example.amukisense.ui.viewmodel.GpsViewModel
import com.example.amukisense.services.GpsCaptureService
...
fun GpsScreen(
    viewModel: GpsViewModel = viewModel(
        factory = GpsViewModel.Factory((LocalContext.current.applicationContext as AmukiSenseApp).gpsRepository)
    )
) {
    ...
    // Estado local para saber si el servicio está corriendo (Emulado para la UI)
    ...
    Column(
        ...
        // Bloqueo temprano: Si no hay permisos concedidos, muestra pantalla de error
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    if (record.google != null) {
                        ...
                        Text("Buscando...", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    if (record.sensors != null) {
                        if (record.sensors.latitude != null) {
                            ...
                            Text("Indoors / Sin Fix", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.error)
                        }
                    } else {
                        Text("Buscando...", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
